...
void MonteCarlo(int N, double& result, double& error, double& spread) {
	double s = M_PI_4 / 2.0;
	double V = gsl_sf_pow_int(s, 8);
	double sum = 0.0;
	double sumSquares = 0.0;
	double minResult = 10000.0;
	double maxResult = -10000.0;
	for (int i = 0; i < N; i++) {
	...
	spread = gsl_sf_pow_int(10.0, 6) * spread;
}
...
int main() {
	...
	for (int i=1; i<=800; i=i+10) {
		...
		for (int n=0; n<100; n++) {
			...
			MonteCarlo(i, currentResult, currentError, currentSpread);
			result += currentResult;
			error += gsl_sf_pow_int(currentError, 2) / (double)i;
			spread += gsl_sf_pow_int(currentSpread, 2) / (double)i;
		}
		result = result / 100.0;
		error = sqrt(error) / 100.0;
		spread = sqrt(spread) / 100.0;
		cout << i << " " << error << " " << spread << "\n";
	}
	return 0;
}
